...
function StatTile({ icon, label, value, pulse }: { icon: React.ReactNode; label: string; value: string; pulse: number }) {
  return (
    <div className="min-w-[78px] rounded-xl bg-secondary/40 border border-primary/15 px-3 py-2 text-center">
      <div className="flex items-center justify-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground">
        {icon}<span>{label}</span>
      </div>
      <div key={`${label}-${pulse}`} className="text-sm font-bold mt-0.5 animate-fade-in">{value}</div>
    </div>
  );
}
